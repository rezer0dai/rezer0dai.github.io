...
.. as you can see DDPG is somehow fast to understand what it should do from begining, i guess better gradient signal, but as learning progress it can stagnate on 'local minima' pretty long time.

.. on the otherside, you can see, that PPO is way too slow to catch up what todo at begining but do quite leaps once it figure it out, and can converge blazingly fast. Actually this can be quite biased output on PPO vs Reacher, as i used bit [different PPO implementation](https://rezer0dai.github.io/test-post/#ppo), though in this case it actually helps to demonstrate potential of idea for cross cooperation works.

{:refdef: style="text-align: center;"}
![cross coop](https://rezer0dai.github.io/assets/images/coop.png "cooperation schema")
{: refdef}
...
More detailed results of test you can check in [notebooks](https://rezer0dai.github.io/test-post/#jupyter-notebooks).
...
class PPO(nn.Module):
    ...
    def forward(self, mu):
        ...
        return dist
```
  - second as DDPG using soft actor critic, implementation will be easier if PPO do the same. And it appears to work.
  - last issue was replay buffer, as on every implementation i saw does not use it directly, they just use buffer to keep memorized all experience from last learning loop to current one and then do random rollouts. But on the other side, at implementation level it is the same as having replay buffer as small to cover exactly that amount of experience. Actually i doubled buffer size to contain experience time period from last two learning steps up to current one, and enabled td error prioritization as, for experimenting, why not. Well, actually it screams of why not ([on vs off](https://leimao.github.io/blog/RL-On-Policy-VS-Off-Policy/), [ddpg maxQ](https://spinningup.openai.com/en/latest/algorithms/ddpg.html, [ppo explained](https://medium.com/@jonathan_hui/rl-proximal-policy-optimization-ppo-explained-77f014ec3f12) -> [ppo is on](https://spinningup.openai.com/en/latest/algorithms/ppo.html))) as i make it more and more off-policy oriented. On the other side, this soft-actor-critic feature can be disabled pretty much easy with setting **tau=1.** and synchronizing networks per update via **sync_delta=1**. I tested with tau=1e-3 and sync_delta=3 especially for purpose of cross-algorithm cooperation in method where i want to sync only explorer networks and let target networks adapt via own learning, so running ppo also in this settings can better shows learning difference.
  - i have enabled one more experimental method, that is when ratio of PPO (new prob : old prob) is way too off to be reasonable i exchange it with REINFORCE signal for backprop for particular experiencec replay (not whole batch). This one not properly tested as it appears does not happening so often. Similiar stuff for advantage_boost in case of advantage gives too little signal, this one also not tested properly, but you can check both ideas implementation [here](https://github.com/rezer0dai/rewheeler/utils/policy.py). Those ones are highly experimental, just to satisfy curiosity of mine.
...
For checking my results / experiments you can follow rewheeler repo. In Particular :
  ...
  - [PPO](https://github.com/rezer0dai/rewheeler/PPO.ipynb)
