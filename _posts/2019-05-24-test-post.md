...
However this technique has little drawback aa we can not simply use n-step estimators when it comes to td-error, which seeks balanca between bias vs variance. Why is so ? Once we setup new goal and trying to calculate td-error \ advantage it is ok for 1-step lookahead, as from both current state and imidiate next state we ask current policy to get us new action taken with respect to specific goal. However if we try to do it with 2+ state, then those states between current and n-state was taken with respect to different goals aka next_state which was achieved with n-steps from cureent state is not usable for td-error under different goal. But then, can we introduce back n-state approximation ?
### Floating n-step
