...
![alt text](https://github.com/rezer0dai/rezer0dai.github.io/assets/images/td_lambda_backward.jpg "GAE backward view")
...
In this section i will be really brief just introduce some features and points out to the source.
### PPO
...
```
  ...
  - last issue was replay buffer, as on every implementation i saw does not use it directly, they just use buffer to keep memorized all experience from last learning loop to current one and then do random rollouts. But on the other side it is the same as having replay buffer as small to cover exactly that amount of experience. Actually i doubled buffer size to contain experience time period from last two learning steps up to current one, and enabled td error prioritization as, for experimenting, why not.
  - i have enabled one more experimental method, that is when ratio of PPO (new prob / old prob) is way too off to be reasonable i exchange it with REINFORCE signal for backprop for particular experiencec replay (not whole batch). This one not properly tested as it appears does not happening so often. Similiar stuff for advantage_boost in case of advantage gives too little signal, this one also not tested properly, but you can check both ideas implementation [here](https://github.com/rezer0dai/rewheeler/utils/policy.py).
