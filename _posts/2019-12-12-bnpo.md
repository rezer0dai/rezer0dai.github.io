...
Why it works for [DDPG](https://arxiv.org/abs/1509.02971) then ? Because here policy objective is take action which maximize expected reward for given state action pair directly ([Bellman equations](https://classroom.udacity.com/courses/ud600/lessons/4436560172/concepts/44332502820923)), instead of doing expectation over policy distribution :
...
However even following configuration does not cover second requirement, as PPO is too efficient for this environment comparsion to DDPG, at least i was not able to tune DDPG to be efficient enough, and almost with any PPO configuration it simple outperforms DDPG by quite a margin. That is one of the reason I need to switch to different environment where PPO struggle is unable to solve (when combine with HER for example), like those from MuJuCo, though I need to work there abstraction layers for better scaling of mujuco environments and get a licence to test it more efficiently, as for that it is on my TODO list. Therefore this setup just shows that those two algorihm can coverge to sucessfull result. 
...
Above mentioned notebooks :
  ...
  - [PPO no HER but FS](https://github.com/rezer0dai/bnpo/blob/master/PPO_no_her_butfs.ipynb) w/o her but with floating step
